...
public class Stepdefs {
    ...
    public void Weather_API_Information_Is_Called_For_City(String city) throws Throwable {
        ...
        String temp = String.valueOf(outputString);

        File f=new File(".\\reports\\test-report\\OutputResult.txt");
        /*
        if (!f.exists())
            f.createNewFile();
         */
        ...
        /*
        File w=new File(".\\reports\\test-report\\OutputResult.txt");
        BufferedWriter out=new BufferedWriter(new FileWriter(w,true));
        out.write(temp);
        out.newLine();
        out.flush();
        */

        File w=new File(".\\reports\\test-report\\oldOutputResult.txt");
        BufferedWriter out=new BufferedWriter(new FileWriter(w,true));
        out.write(temp);
        out.newLine();
        out.close();
        boolean success = (new File
                (".\\reports\\test-report\\oldOutputResult.txt")).delete();
        if (success) {
            System.out.println("The file has been successfully deleted");

        }

        File f1=new File(".\\reports\\test-report\\newOutputResult.txt");
        f1.createNewFile();

        File w1=new File(".\\reports\\test-report\\newOutputResult.txt");
        BufferedWriter out1=new BufferedWriter(new FileWriter(w1,true));
        out1.write(temp);
        out1.newLine();
        out1.close();
